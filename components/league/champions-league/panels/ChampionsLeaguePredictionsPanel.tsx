...
function Stat({
  ...
  return (
    ...
    >
      <span className="text-[0.65rem] font-medium uppercase tracking-wide text-[#f1f3f8]/50">
        {label}
      </span>
...
function PredictionCard({ row }: { row: PredictionCardMock }) {
  ...
  return (
    ...
    >
      <div className="mb-4 border-b pb-3" style={{ borderColor: UCL_BORDER }}>
        <div className="mb-3 flex flex-wrap items-center justify-end gap-2">
          <span
            className="shrink-0 rounded-full px-3 py-1 text-xs font-bold sm:text-sm"
            style={{
              backgroundColor: isLive ? "rgba(0,238,255,0.18)" : "rgba(241,243,248,0.08)",
              color: isLive ? uclAccentCyan : "rgba(241,243,248,0.88)",
            }}
          >
        ...
        {isScheduled && row.matchKickoffLabel ? (
          <p className="mb-3 text-center text-sm font-medium text-[#f1f3f8]/65 sm:text-base">
        ...
        <div className="flex items-center justify-center gap-4 sm:gap-6">
          <div className="flex flex-col items-center gap-1.5">
            ...
            <span className="max-w-[5.5rem] text-center text-[11px] font-medium leading-snug text-[#f1f3f8]/60 sm:max-w-[6.5rem] sm:text-xs">
        ...
        <p className="mt-3 text-center text-xs text-[#f1f3f8]/55 sm:text-sm">
          Predicción asociada a{" "}
          <span className="font-bold text-[#f1f3f8]">{row.teamForPrediction}</span>
        </p>
      </div>

      <p className="mb-4 text-xs text-[#f1f3f8]/50">
        Generada:{" "}
...
export function ChampionsLeaguePredictionsPanel() {
  return (
    <div className="space-y-8">
      ...
      >
        <h2 className="text-lg font-bold text-[#f1f3f8] sm:text-xl">Predicciones</h2>
        <p className="text-sm leading-relaxed text-[#f1f3f8]/70 sm:text-base">
          Aquí verás las <strong className="text-[#f1f3f8]">predicciones por equipo</strong>{" "}
          para los partidos que tengan modelo disponible. Solo aplican a encuentros{" "}
          <strong className="text-[#f1f3f8]">por jugar</strong> o{" "}
          <strong style={{ color: uclAccentBlue }}>en vivo</strong>: los partidos{" "}
          <strong className="text-[#f1f3f8]">ya jugados no aparecen en esta lista</strong>.
          Las predicciones de partidos <strong className="text-[#f1f3f8]">ya jugados</strong>{" "}
          se verán en la <strong className="text-[#f1f3f8]">vista propia de cada partido</strong>.
