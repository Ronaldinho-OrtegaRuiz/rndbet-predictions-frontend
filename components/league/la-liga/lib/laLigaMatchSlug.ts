import { LA_LIGA_FIXTURE_PAIRS } from "../mocks/laLigaMatchdayMock";

/** Slug por nombre de club (luego por id). */
...
export function tryParseMatchSlug(
  ...
  const idx = LA_LIGA_FIXTURE_PAIRS.findIndex(
    (p) =>
      slugifyTeamName(p.home) === homeSlug && slugifyTeamName(p.away) === awaySlug,
  );
  if (idx === -1) return null;
  const p = LA_LIGA_FIXTURE_PAIRS[idx]!;
