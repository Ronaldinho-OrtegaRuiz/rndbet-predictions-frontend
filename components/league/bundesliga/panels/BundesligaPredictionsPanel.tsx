...
} from "../mocks/bundesligaPredictionsMock";
...
function PredictionCard({ row }: { row: PredictionCardMock }) {
  ...
  return (
    ...
    >
      <div className="mb-4 border-b border-[#34383c] pb-3">
        <div className="mb-3 flex flex-wrap items-center justify-end gap-2">
          <span
            className="shrink-0 rounded-full px-3 py-1 text-xs font-bold sm:text-sm"
            style={{
              backgroundColor: isLive ? `${bundesligaAccent}24` : "rgba(255,255,255,0.06)",
              color: isLive ? bundesligaAccent : "rgba(226, 232, 240, 0.85)",
            }}
          >
        ...
        {isScheduled && row.matchKickoffLabel ? (
          <p className="mb-3 text-center text-sm font-medium text-slate-400 sm:text-base">
        ...
        <div className="flex items-center justify-center gap-4 sm:gap-6">
          <div className="flex flex-col items-center gap-1.5">
            ...
            <span className="max-w-[5.5rem] text-center text-[11px] font-medium leading-snug text-slate-400 sm:max-w-[6.5rem] sm:text-xs">
              {row.teamForPrediction}
            </span>
          </div>
          <span className="shrink-0 text-sm font-bold tracking-wide text-slate-500 sm:text-base">
            VS
          </span>
          <div className="flex flex-col items-center gap-1.5">
        ...
        <p className="mt-3 text-center text-xs text-slate-500 sm:text-sm">
          ...
          <span className="font-bold text-slate-100">{row.teamForPrediction}</span>
        </p>
      </div>
...
export function BundesligaPredictionsPanel() {
  return (
    <div className="space-y-8">
      ...
      >
        <h2 className="text-lg font-bold text-slate-100 sm:text-xl">Predicciones</h2>
        <p className="text-sm leading-relaxed text-slate-400 sm:text-base">
          Aquí verás las <strong className="text-slate-200">predicciones por equipo</strong>{" "}
          para los partidos que tengan modelo disponible. Solo aplican a encuentros{" "}
          <strong className="text-slate-200">por jugar</strong> o{" "}
          <strong style={{ color: bundesligaAccent }}>en vivo</strong>: los partidos{" "}
          <strong className="text-slate-200">ya jugados no aparecen en esta lista</strong>.
          Las predicciones de partidos <strong className="text-slate-200">ya jugados</strong>{" "}
          se verán en la <strong className="text-slate-200">vista propia de cada partido</strong>.
