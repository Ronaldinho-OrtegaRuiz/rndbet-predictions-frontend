...
import { matchSlugFromTeams } from "../lib/serieAMatchSlug";
import { MATCHDAYS } from "../mocks/serieAMatchdayMock";
import type { FixtureMatch } from "../mocks/serieAMatchdayMock";
...
function FixtureCard({ match }: { match: FixtureMatch }) {
  ...
  const href = `/league/serie-a/match/${matchSlugFromTeams(homeTeam, awayTeam)}`;

  return (
    <Link
      href={href}
      className="relative block rounded-2xl border border-white/[0.08] bg-[#15365a] px-4 py-4 shadow-[0_6px_24px_rgba(0,0,0,0.18)] transition-colors hover:border-white/[0.12] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#0e8efd]/50 sm:px-5 sm:py-5"
    >
...
export function SerieAMatchdayPanel() {
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-center gap-3">
        <label htmlFor="seriea-matchday" className="text-sm font-bold text-white/80">
          Jornada
        </label>
        <select
          id="seriea-matchday"
          ...
          className="cursor-pointer rounded-xl border border-white/[0.12] bg-[#15365a] px-4 py-2.5 text-base font-semibold text-white shadow-inner outline-none transition-colors hover:border-white/20 focus-visible:ring-2 focus-visible:ring-[#0e8efd]/60"
        >
