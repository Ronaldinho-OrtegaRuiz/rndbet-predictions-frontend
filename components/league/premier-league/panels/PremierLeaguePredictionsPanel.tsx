...
} from "../mocks/premierLeaguePredictionsMock";
...
function Stat({
  ...
  return (
    <div className="flex flex-col gap-0.5 rounded-lg border border-white/[0.06] bg-[#1e0021]/50 px-3 py-2">
      <span className="text-[0.65rem] font-medium uppercase tracking-wide text-white/45">
        {label}
      </span>
      <span className="text-sm font-semibold tabular-nums text-white/95 sm:text-base">
        {value}
      </span>
    </div>
  );
...
function PredictionCard({ row }: { row: PredictionCardMock }) {
  ...
  return (
    <article className="rounded-2xl border border-white/[0.08] bg-[#28002b] p-4 shadow-[0_6px_24px_rgba(0,0,0,0.18)] sm:p-5">
      <div className="mb-4 border-b border-white/[0.06] pb-3">
        <div className="mb-3 flex flex-wrap items-center justify-end gap-2">
          <span
            ...
            style={{
              backgroundColor: isLive ? `${premierLeagueAccent}22` : "rgba(255,255,255,0.08)",
              color: isLive ? premierLeagueAccent : "rgba(255,255,255,0.75)",
            }}
          >
        ...
        {isScheduled && row.matchKickoffLabel ? (
          <p className="mb-3 text-center text-sm font-medium text-white/65 sm:text-base">
        ...
        <div className="flex items-center justify-center gap-4 sm:gap-6">
          <div className="flex flex-col items-center gap-1.5">
            ...
            <span className="max-w-[5.5rem] text-center text-[11px] font-medium leading-snug text-white/60 sm:max-w-[6.5rem] sm:text-xs">
              {row.teamForPrediction}
            </span>
          </div>
          <span className="shrink-0 text-sm font-bold tracking-wide text-white/35 sm:text-base">
            VS
          </span>
          <div className="flex flex-col items-center gap-1.5">
        ...
        <p className="mt-3 text-center text-xs text-white/50 sm:text-sm">
          ...
          <span className="font-bold text-white/90">{row.teamForPrediction}</span>
        </p>
      </div>
...
export function PremierLeaguePredictionsPanel() {
  return (
    <div className="space-y-8">
      <div className="space-y-3 rounded-2xl border border-white/[0.08] bg-[#28002b]/60 p-4 sm:p-6">
        <h2 className="text-lg font-bold text-white sm:text-xl">Predicciones</h2>
        <p className="text-sm leading-relaxed text-white/70 sm:text-base">
          Aquí verás las <strong className="text-white">predicciones por equipo</strong>{" "}
          para los partidos que tengan modelo disponible. Solo aplican a encuentros{" "}
          <strong className="text-white">por jugar</strong> o{" "}
          <strong className="text-white">en vivo</strong>: los partidos{" "}
          <strong className="text-white">ya jugados no aparecen en esta lista</strong>.
          Las predicciones de partidos <strong className="text-white">ya jugados</strong>{" "}
          se verán en la <strong className="text-white">vista propia de cada partido</strong>.
