...
function LogoSlot({ label }: { label: string }) {
  return (
    <span
      className="flex size-14 shrink-0 items-center justify-center rounded-xl border border-[#34383c] bg-[#1a2128] shadow-sm sm:size-16 md:size-[4.5rem]"
      aria-hidden
      title={label}
    />
  );
...
function Timeline({ events, homeTeam, awayTeam }: { events: MatchEventMock[]; homeTeam: string; awayTeam: string }) {
  ...
  if (sorted.length === 0) {
    return (
      <div className="rounded-2xl border border-[#34383c] bg-[#1a2128] p-4 text-sm text-slate-400 sm:p-5">
        No hay eventos en el mock para este partido. Con el back, aquí aparecerá la línea de tiempo desde{" "}
        <code className="text-slate-200">match_events</code>.
      </div>
    );
  }

  return (
    <div className="rounded-2xl border border-[#34383c] bg-[#1a2128] p-4 shadow-sm sm:p-5">
      <h3 className="mb-4 text-sm font-bold text-slate-100 sm:text-base">Eventos</h3>
      <div className="relative pl-8">
        <span
          className="absolute bottom-2 left-[0.65rem] top-2 w-px bg-[#34383c]"
          aria-hidden
        />
        ...
        {sorted.map((ev) => {
          ...
          return (
            <li key={ev.id} className="relative pb-6 last:pb-0">
              <span
                className="absolute left-0 top-1.5 flex size-5 items-center justify-center rounded-full border-2 border-[#1a2128] shadow-sm sm:top-2"
                style={{
                  backgroundColor:
                    ev.side === "home" ? `${bundesligaAccent}40` : "rgba(148, 163, 184, 0.45)",
                }}
                aria-hidden
              />
              <div className="flex flex-wrap items-baseline gap-x-2 gap-y-0.5">
                <span className="text-xs font-bold tabular-nums text-slate-500 sm:text-sm">
                  {ev.minute != null ? `${ev.minute}'` : "—"}
                </span>
                <span className="text-xs font-bold uppercase tracking-wide text-slate-300 sm:text-sm">
                  {eventLabel(ev.event_type)}
                </span>
                <span className="text-[11px] text-slate-400 sm:text-xs">· {team}</span>
              </div>
              {ev.playerLabel ? (
                <p className="mt-1 text-sm font-medium text-slate-100">{ev.playerLabel}</p>
              ) : null}
...
function StatsCompare({ home, away }: { home: TeamMatchStatsMock; away: TeamMatchStatsMock }) {
  return (
    <div className="space-y-1">
      <div className="mb-3 grid grid-cols-[1fr_auto_1fr] gap-2 text-center text-xs font-bold text-slate-500 sm:text-sm">
        <span className="truncate text-right text-slate-100">{home.teamName}</span>
        <span className="shrink-0 px-1" />
        <span className="truncate text-left text-slate-100">{away.teamName}</span>
      </div>
      {STAT_ROWS.map(({ key, label, format }) => {
        ...
        return (
          <div
            key={String(key)}
            className="grid grid-cols-[1fr_auto_1fr] items-center gap-x-2 gap-y-1 rounded-lg border border-[#34383c] bg-[#0f1419] px-2 py-2 sm:px-3"
          >
            <div className="flex flex-col items-end gap-1">
              <span className="text-sm font-semibold tabular-nums text-slate-100 sm:text-base">
                {formatStatValue(hn, format)}
              </span>
              <div className="h-1 w-full max-w-[6rem] overflow-hidden rounded-full bg-[#34383c]">
                <div
                  className="h-full rounded-full transition-all"
                  style={{ width: `${hw}%`, backgroundColor: bundesligaAccent }}
                />
              </div>
            </div>
            <span className="min-w-[5.5rem] text-center text-[0.65rem] font-medium uppercase tracking-wide text-slate-500 sm:min-w-[6.5rem] sm:text-xs">
              {label}
            </span>
            <div className="flex flex-col items-start gap-1">
              ...
              <div className="h-1 w-full max-w-[6rem] overflow-hidden rounded-full bg-[#34383c]">
                <div
                  className="h-full rounded-full bg-[#6b7280] transition-all"
                  style={{ width: `${aw}%` }}
                />
              </div>
            </div>
...
function PredictionBlock({
  ...
  return (
    <div className="space-y-6">
      <section>
        <h4 className="mb-3 text-xs font-bold uppercase tracking-wide text-slate-500">Predicción</h4>
        <div className="grid gap-2 sm:grid-cols-2">
          <div className="rounded-lg border border-[#34383c] bg-[#0f1419] px-3 py-2">
            <span className="text-[0.65rem] font-medium uppercase text-slate-500">Goles esperados</span>
          ...
          <div className="rounded-lg border border-[#34383c] bg-[#0f1419] px-3 py-2">
            <span className="text-[0.65rem] font-medium uppercase text-slate-500">1X2</span>
            <p className="text-sm font-semibold text-slate-100">
              {pct(prediction.prob_home_win)} / {pct(prediction.prob_draw)} /{" "}
              {pct(prediction.prob_away_win)}
            </p>
          </div>
          <div className="rounded-lg border border-[#34383c] bg-[#0f1419] px-3 py-2 sm:col-span-2">
            <span className="text-[0.65rem] font-medium uppercase text-slate-500">Conteos previstos</span>
      ...
      {evaluation ? (
        <section>
          <h4 className="mb-3 text-xs font-bold uppercase tracking-wide text-slate-500">Evaluación</h4>
          <div className="grid gap-2 sm:grid-cols-2">
            <div className="rounded-lg border border-[#34383c] bg-[#0f1419] px-3 py-2">
              <span className="text-[0.65rem] font-medium uppercase text-slate-500">Marcador real</span>
              <p className="text-sm font-semibold text-slate-100">
                {evaluation.actual_home_goals ?? "—"} — {evaluation.actual_away_goals ?? "—"}
              </p>
            </div>
            <div className="rounded-lg border border-[#34383c] bg-[#0f1419] px-3 py-2">
              <span className="text-[0.65rem] font-medium uppercase text-slate-500">Resultado acertado</span>
              <p className="text-sm font-semibold text-slate-100">
            ...
            <div className="rounded-lg border border-[#34383c] bg-[#0f1419] px-3 py-2 sm:col-span-2">
              <span className="text-[0.65rem] font-medium uppercase text-slate-500">Valores reales</span>
              <p className="mt-1 text-xs leading-relaxed text-slate-300 sm:text-sm">
            ...
            <div className="rounded-lg border border-[#34383c] bg-[#0f1419] px-3 py-2 sm:col-span-2">
              <span className="text-[0.65rem] font-medium uppercase text-slate-500">Errores agregados</span>
        ...
        <p className="text-sm text-slate-400">
          Sin fila en <code className="text-slate-200">prediction_evaluations</code> en el mock; con el back se
...
export function BundesligaMatchDetailView({
  ...
  const top =
    status === "live" ? (
      ...
      <p className="text-center text-sm text-slate-400 sm:text-base">{dateLabel ?? "Fecha por confirmar"}</p>
    ) : (
      <p className="text-center text-sm text-slate-500 sm:text-base">{dateLabel ?? "Finalizado"}</p>
    );
  ...
  return (
    <div className="space-y-8">
      <article className="rounded-2xl border border-[#34383c] bg-[#1a2128] p-4 shadow-sm sm:p-6 md:p-8">
        ...
        <div className="flex flex-wrap items-start justify-center gap-4 sm:gap-8 md:gap-12">
          <div className="flex min-w-0 flex-1 flex-col items-center gap-3">
            ...
            <p className="text-center text-sm font-bold text-slate-100 sm:text-base md:text-lg">{homeTeam}</p>
          ...
          <div className="flex flex-col items-center justify-center gap-2 pt-4 sm:pt-8">
            <span className="text-lg font-bold tracking-[0.2em] text-slate-400 sm:text-xl">VS</span>
          </div>
          <div className="flex min-w-0 flex-1 flex-col items-center gap-3">
            <LogoSlot label={awayTeam} />
            <p className="text-center text-sm font-bold text-slate-100 sm:text-base md:text-lg">{awayTeam}</p>
            <p className="text-4xl font-bold tabular-nums text-slate-100 sm:text-5xl">
              {showScore ? awayScore ?? "—" : "—"}
            </p>
          </div>
      ...
      <div className="grid gap-6 lg:grid-cols-[minmax(0,280px)_1fr] lg:items-start">
        ...
        <div className="order-1 min-w-0 lg:order-2">
          <div className="rounded-2xl border border-[#34383c] bg-[#1a2128] p-4 shadow-sm sm:p-6">
            <div className="mb-5 flex flex-wrap items-center justify-center gap-2 sm:justify-start">
              <span className="mr-0 text-xs font-medium text-slate-500 sm:mr-2 sm:text-sm">Ver:</span>
              <div className="inline-flex rounded-full border border-[#34383c] bg-[#0f1419] p-1">
                <button
                  type="button"
                  ...
                  style={
                    sideTab === "stats"
                      ? { backgroundColor: bundesligaAccent }
            ...
            {sideTab === "stats" ? (
              <>
                <h3 className="mb-4 text-base font-bold text-slate-100 sm:text-lg">Estadísticas del partido</h3>
                <StatsCompare home={detail.homeStats} away={detail.awayStats} />
                <footer className="mt-6 rounded-xl border border-dashed border-[#34383c] bg-[#0f1419] p-4 text-sm leading-relaxed text-slate-400">
                  {detail.statsFootnote}
                </footer>
              </>
            ) : (
              <>
                <h3 className="mb-4 text-base font-bold text-slate-100 sm:text-lg">Predicción y evaluación</h3>
                {detail.prediction ? (
                  ...
                  <p className="text-sm text-slate-400">
                    ...
                    <code className="text-slate-200">predictions</code> y{" "}
                    <code className="text-slate-200">prediction_evaluations</code>.
                  </p>
                )}
                <footer className="mt-6 rounded-xl border border-dashed border-[#34383c] bg-[#0f1419] p-4 text-sm leading-relaxed text-slate-400">
