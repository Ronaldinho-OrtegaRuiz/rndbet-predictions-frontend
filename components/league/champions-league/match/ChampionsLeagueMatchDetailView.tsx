...
function Timeline({ events, homeTeam, awayTeam }: { events: MatchEventMock[]; homeTeam: string; awayTeam: string }) {
  ...
  if (sorted.length === 0) {
    return (
      <div
        className="rounded-2xl border p-4 text-sm text-[#f1f3f8]/65 sm:p-5"
        style={{ borderColor: UCL_BORDER, backgroundColor: UCL_BG_SURFACE }}
      >
        No hay eventos en el mock para este partido. Con el back, aquí aparecerá la línea de tiempo desde{" "}
        <code className="text-[#f1f3f8]/90">match_events</code>.
      </div>
    );
  }
  ...
  return (
    <div
      ...
      style={{ borderColor: UCL_BORDER, backgroundColor: UCL_BG_SURFACE }}
    >
      <h3 className="mb-4 text-sm font-bold text-[#f1f3f8] sm:text-base">Eventos</h3>
      <div className="relative pl-8">
        <span
          className="absolute bottom-2 left-[0.65rem] top-2 w-px"
          style={{ backgroundColor: UCL_BORDER }}
          aria-hidden
        />
          ...
          return (
            <li key={ev.id} className="relative pb-6 last:pb-0">
              <span
                ...
                style={{
                  borderColor: UCL_BG_SURFACE,
                  backgroundColor:
                    ev.side === "home" ? `${uclAccentBlue}66` : `${uclAccentCyan}55`,
                }}
                aria-hidden
              />
              <div className="flex flex-wrap items-baseline gap-x-2 gap-y-0.5">
                <span className="text-xs font-bold tabular-nums text-[#f1f3f8]/50 sm:text-sm">
                  {ev.minute != null ? `${ev.minute}'` : "—"}
                </span>
                <span className="text-xs font-bold uppercase tracking-wide text-[#f1f3f8]/80 sm:text-sm">
                  {eventLabel(ev.event_type)}
                </span>
                <span className="text-[11px] text-[#f1f3f8]/55 sm:text-xs">· {team}</span>
              </div>
              {ev.playerLabel ? (
                <p className="mt-1 text-sm font-medium text-[#f1f3f8]">{ev.playerLabel}</p>
              ) : null}
              {sub ? <p className="mt-0.5 text-xs text-[#f1f3f8]/65">{sub}</p> : null}
            </li>
          );
        })}
        </ul>
...
function StatsCompare({ home, away }: { home: TeamMatchStatsMock; away: TeamMatchStatsMock }) {
  return (
    <div className="space-y-1">
      <div className="mb-3 grid grid-cols-[1fr_auto_1fr] gap-2 text-center text-xs font-bold text-[#f1f3f8]/50 sm:text-sm">
        <span className="truncate text-right text-[#f1f3f8]">{home.teamName}</span>
        <span className="shrink-0 px-1" />
        <span className="truncate text-left text-[#f1f3f8]">{away.teamName}</span>
      </div>
      {STAT_ROWS.map(({ key, label, format }) => {
        ...
        return (
          <div
            ...
            style={{ borderColor: UCL_BORDER, backgroundColor: UCL_BG_DEEP }}
          >
            <div className="flex flex-col items-end gap-1">
              <span className="text-sm font-semibold tabular-nums text-[#f1f3f8] sm:text-base">
              ...
              >
                <div
                  ...
                  style={{ width: `${hw}%`, backgroundColor: uclAccentBlue }}
                />
              </div>
            </div>
            <span className="min-w-[5.5rem] text-center text-[0.65rem] font-medium uppercase tracking-wide text-[#f1f3f8]/50 sm:min-w-[6.5rem] sm:text-xs">
              {label}
            </span>
            <div className="flex flex-col items-start gap-1">
              ...
              >
                <div
                  className="h-full rounded-full transition-all"
                  style={{ width: `${aw}%`, backgroundColor: uclAccentCyan }}
                />
              </div>
            </div>
          </div>
...
function PredictionBlock({
  ...
  const boxStyle = { borderColor: UCL_BORDER, backgroundColor: UCL_BG_DEEP };
  return (
    <div className="space-y-6">
      <section>
        <h4 className="mb-3 text-xs font-bold uppercase tracking-wide text-[#f1f3f8]/50">Predicción</h4>
        <div className="grid gap-2 sm:grid-cols-2">
          <div className={box} style={boxStyle}>
            <span className="text-[0.65rem] font-medium uppercase text-[#f1f3f8]/50">Goles esperados</span>
          ...
          <div className={box} style={boxStyle}>
            <span className="text-[0.65rem] font-medium uppercase text-[#f1f3f8]/50">1X2</span>
            <p className="text-sm font-semibold text-[#f1f3f8]">
          ...
          <div className={`${box} sm:col-span-2`} style={boxStyle}>
            <span className="text-[0.65rem] font-medium uppercase text-[#f1f3f8]/50">Conteos previstos</span>
            <p className="mt-1 text-xs leading-relaxed text-[#f1f3f8]/75 sm:text-sm">
        ...
        <p className="mt-2 text-[11px] text-[#f1f3f8]/50">
          Creada:{" "}
      ...
      {evaluation ? (
        <section>
          <h4 className="mb-3 text-xs font-bold uppercase tracking-wide text-[#f1f3f8]/50">Evaluación</h4>
          <div className="grid gap-2 sm:grid-cols-2">
            <div className={box} style={boxStyle}>
              <span className="text-[0.65rem] font-medium uppercase text-[#f1f3f8]/50">Marcador real</span>
              <p className="text-sm font-semibold text-[#f1f3f8]">
                {evaluation.actual_home_goals ?? "—"} — {evaluation.actual_away_goals ?? "—"}
              </p>
            </div>
            <div className={box} style={boxStyle}>
              <span className="text-[0.65rem] font-medium uppercase text-[#f1f3f8]/50">Resultado acertado</span>
              <p className="text-sm font-semibold text-[#f1f3f8]">
            ...
            <div className={`${box} sm:col-span-2`} style={boxStyle}>
              <span className="text-[0.65rem] font-medium uppercase text-[#f1f3f8]/50">Valores reales</span>
              <p className="mt-1 text-xs leading-relaxed text-[#f1f3f8]/75 sm:text-sm">
            ...
            <div className={`${box} sm:col-span-2`} style={boxStyle}>
              <span className="text-[0.65rem] font-medium uppercase text-[#f1f3f8]/50">Errores agregados</span>
              <p className="mt-1 text-xs text-[#f1f3f8]/75 sm:text-sm">
        ...
        <p className="text-sm text-[#f1f3f8]/65">
          Sin fila en <code className="text-[#f1f3f8]/90">prediction_evaluations</code> en el mock; con el back se
...
export function ChampionsLeagueMatchDetailView({ detail }: { detail: MatchDetailMock }) {
  ...
  const top =
    status === "live" ? (
      ...
      <p className="text-center text-sm text-[#f1f3f8]/65 sm:text-base">{dateLabel ?? "Fecha por confirmar"}</p>
    ) : (
      <p className="text-center text-sm text-[#f1f3f8]/55 sm:text-base">{dateLabel ?? "Finalizado"}</p>
    );

  const showScore = status !== "scheduled";

  const cardStyle = { borderColor: UCL_BORDER, backgroundColor: UCL_BG_SURFACE };
      ...
      <article className="rounded-2xl border p-4 shadow-sm sm:p-6 md:p-8" style={cardStyle}>
        ...
        <div className="flex flex-wrap items-start justify-center gap-4 sm:gap-8 md:gap-12">
          <div className="flex min-w-0 flex-1 flex-col items-center gap-3">
            ...
            <p className="text-center text-sm font-bold text-[#f1f3f8] sm:text-base md:text-lg">{homeTeam}</p>
            <p className="text-4xl font-bold tabular-nums text-[#f1f3f8] sm:text-5xl">
              {showScore ? homeScore ?? "—" : "—"}
            </p>
          </div>
          ...
          <div className="flex min-w-0 flex-1 flex-col items-center gap-3">
            ...
            <p className="text-center text-sm font-bold text-[#f1f3f8] sm:text-base md:text-lg">{awayTeam}</p>
            <p className="text-4xl font-bold tabular-nums text-[#f1f3f8] sm:text-5xl">
              {showScore ? awayScore ?? "—" : "—"}
            </p>
          </div>
        ...
        <div className="order-1 min-w-0 lg:order-2">
          <div className="rounded-2xl border p-4 shadow-sm sm:p-6" style={cardStyle}>
            <div className="mb-5 flex flex-wrap items-center justify-center gap-2 sm:justify-start">
              <span className="mr-0 text-xs font-medium text-[#f1f3f8]/50 sm:mr-2 sm:text-sm">Ver:</span>
              <div
                ...
                <button
                  ...
                  className={`rounded-full px-4 py-2 text-xs font-bold transition-colors sm:text-sm ${
                    sideTab === "stats"
                      ...
                      : "text-[#f1f3f8]/55 hover:text-[#f1f3f8]"
                  }`}
                  style={
                    sideTab === "stats"
                      ? { backgroundColor: uclAccentBlue }
            ...
            {sideTab === "stats" ? (
              <>
                <h3 className="mb-4 text-base font-bold text-[#f1f3f8] sm:text-lg">Estadísticas del partido</h3>
                <StatsCompare home={detail.homeStats} away={detail.awayStats} />
                <footer
                  className="mt-6 rounded-xl border border-dashed p-4 text-sm leading-relaxed text-[#f1f3f8]/65"
                  style={{ borderColor: UCL_BORDER, backgroundColor: UCL_BG_DEEP }}
                >
              ...
              <>
                <h3 className="mb-4 text-base font-bold text-[#f1f3f8] sm:text-lg">Predicción y evaluación</h3>
                {detail.prediction ? (
                  ...
                  <p className="text-sm text-[#f1f3f8]/65">
                    ...
                    <code className="text-[#f1f3f8]/90">predictions</code> y{" "}
                    <code className="text-[#f1f3f8]/90">prediction_evaluations</code>.
                  </p>
                )}
                <footer
                  className="mt-6 rounded-xl border border-dashed p-4 text-sm leading-relaxed text-[#f1f3f8]/65"
