...
function LogoSlot({ label }: { label: string }) {
  return (
    <span
      className="flex size-14 shrink-0 items-center justify-center rounded-xl border border-slate-200 bg-white shadow-sm sm:size-16 md:size-[4.5rem]"
      aria-hidden
      title={label}
    />
  );
...
function Timeline({ events, homeTeam, awayTeam }: { events: MatchEventMock[]; homeTeam: string; awayTeam: string }) {
  ...
  if (sorted.length === 0) {
    return (
      <div className="rounded-2xl border border-slate-200 bg-white p-4 text-sm text-slate-600 sm:p-5">
        ...
        <code className="text-slate-800">match_events</code>.
      </div>
    );
  }
  ...
  return (
    <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:p-5">
      <h3 className="mb-4 text-sm font-bold text-slate-900 sm:text-base">Eventos</h3>
      <div className="relative pl-8">
        <span
          className="absolute bottom-2 left-[0.65rem] top-2 w-px bg-slate-200"
          aria-hidden
        />
        ...
        {sorted.map((ev) => {
          ...
          return (
            <li key={ev.id} className="relative pb-6 last:pb-0">
              <span
                className="absolute left-0 top-1.5 flex size-5 items-center justify-center rounded-full border-2 border-white shadow-sm sm:top-2"
                style={{
                  backgroundColor:
                    ev.side === "home" ? `${laLigaAccent}40` : "rgba(148, 163, 184, 0.45)",
                }}
                aria-hidden
              />
              ...
              {ev.playerLabel ? (
                <p className="mt-1 text-sm font-medium text-slate-900">{ev.playerLabel}</p>
              ) : null}
  ...
  return (
    <div className="space-y-1">
      <div className="mb-3 grid grid-cols-[1fr_auto_1fr] gap-2 text-center text-xs font-bold text-slate-500 sm:text-sm">
        <span className="truncate text-right text-slate-900">{home.teamName}</span>
        <span className="shrink-0 px-1" />
        <span className="truncate text-left text-slate-900">{away.teamName}</span>
      </div>
      {STAT_ROWS.map(({ key, label, format }) => {
        ...
        return (
          <div
            key={String(key)}
            className="grid grid-cols-[1fr_auto_1fr] items-center gap-x-2 gap-y-1 rounded-lg border border-slate-200 bg-slate-50/80 px-2 py-2 sm:px-3"
          >
            <div className="flex flex-col items-end gap-1">
              <span className="text-sm font-semibold tabular-nums text-slate-900 sm:text-base">
                {formatStatValue(hn, format)}
              </span>
              <div className="h-1 w-full max-w-[6rem] overflow-hidden rounded-full bg-slate-200">
                <div
                  className="h-full rounded-full transition-all"
                  style={{ width: `${hw}%`, backgroundColor: laLigaAccent }}
                />
              </div>
            </div>
    ...
    <div className="space-y-6">
      ...
      {evaluation ? (
        ...
        <p className="text-sm text-slate-600">
          Sin fila en <code className="text-slate-800">prediction_evaluations</code> en el mock; con el back se
...
export function LaLigaMatchDetailView({ detail }: { detail: MatchDetailMock }) {
  ...
  const top =
    status === "live" ? (
      <div className="flex items-center justify-center gap-2 text-sm font-semibold sm:text-base">
        <span style={{ color: laLigaAccent }}>En vivo</span>
        <span className="tabular-nums text-slate-800">{minute}</span>
      </div>
    ) : status === "scheduled" ? (
      <p className="text-center text-sm text-slate-600 sm:text-base">{dateLabel ?? "Fecha por confirmar"}</p>
    ) : (
      ...
      <article className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:p-6 md:p-8">
        ...
        <div className="flex flex-wrap items-start justify-center gap-4 sm:gap-8 md:gap-12">
          <div className="flex min-w-0 flex-1 flex-col items-center gap-3">
            ...
            <p className="text-center text-sm font-bold text-slate-900 sm:text-base md:text-lg">{homeTeam}</p>
            <p className="text-4xl font-bold tabular-nums text-slate-900 sm:text-5xl">
              {showScore ? homeScore ?? "—" : "—"}
            </p>
          </div>
          ...
          <div className="flex min-w-0 flex-1 flex-col items-center gap-3">
            ...
            <p className="text-center text-sm font-bold text-slate-900 sm:text-base md:text-lg">{awayTeam}</p>
            <p className="text-4xl font-bold tabular-nums text-slate-900 sm:text-5xl">
              {showScore ? awayScore ?? "—" : "—"}
            </p>
          </div>
      ...
      <div className="grid gap-6 lg:grid-cols-[minmax(0,280px)_1fr] lg:items-start">
        ...
        <div className="order-1 min-w-0 lg:order-2">
          <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:p-6">
            ...
            {sideTab === "stats" ? (
              <>
                <h3 className="mb-4 text-base font-bold text-slate-900 sm:text-lg">Estadísticas del partido</h3>
                <StatsCompare home={detail.homeStats} away={detail.awayStats} />
                <footer className="mt-6 rounded-xl border border-dashed border-slate-200 bg-slate-50 p-4 text-sm leading-relaxed text-slate-600">
                  {detail.statsFootnote}
                </footer>
              </>
            ) : (
              <>
                <h3 className="mb-4 text-base font-bold text-slate-900 sm:text-lg">Predicción y evaluación</h3>
                {detail.prediction ? (
                  ...
                  <p className="text-sm text-slate-600">
                    ...
                    <code className="text-slate-800">predictions</code> y{" "}
                    <code className="text-slate-800">prediction_evaluations</code>.
                  </p>
                )}
                <footer className="mt-6 rounded-xl border border-dashed border-slate-200 bg-slate-50 p-4 text-sm leading-relaxed text-slate-600">
