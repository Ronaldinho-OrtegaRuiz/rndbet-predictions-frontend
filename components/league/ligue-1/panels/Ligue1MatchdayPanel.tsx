...
import { matchSlugFromTeams } from "../lib/ligue1MatchSlug";
import { MATCHDAYS } from "../mocks/ligue1MatchdayMock";
import type { FixtureMatch } from "../mocks/ligue1MatchdayMock";
...
function FixtureCard({ match }: { match: FixtureMatch }) {
  ...
  const href = `/league/ligue-1/match/${matchSlugFromTeams(homeTeam, awayTeam)}`;

  return (
    <Link
      href={href}
      className="relative block rounded-2xl border px-4 py-4 shadow-sm transition-colors hover:bg-[#2a2a2a] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#085fff]/45 sm:px-5 sm:py-5"
      style={{
        borderColor: LIGUE1_BORDER,
...
export function Ligue1MatchdayPanel() {
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-center gap-3">
        <label htmlFor="ligue1-matchday" className="text-sm font-bold text-slate-200">
          Journée
        </label>
        <select
          id="ligue1-matchday"
          ...
          className="cursor-pointer rounded-xl border px-4 py-2.5 text-base font-semibold text-slate-200 shadow-sm outline-none transition-colors focus-visible:ring-2 focus-visible:ring-[#085fff]/50"
          style={{
            borderColor: LIGUE1_BORDER,
            backgroundColor: LIGUE1_BG_SURFACE,
          }}
